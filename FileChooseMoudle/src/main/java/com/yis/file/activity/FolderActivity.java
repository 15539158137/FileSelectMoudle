...
public class FolderActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.send).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                progressDialog.show();
                List allChoosedData = new ArrayList();
                for (FileInfo fileInfo : wordData) {
                    if (fileInfo.isChoosed()) {
                        allChoosedData.add(fileInfo);
                    }
                }
                for (FileInfo fileInfo : xlsData) {
                    if (fileInfo.isChoosed()) {
                        allChoosedData.add(fileInfo);
                    }
                }
                for (FileInfo fileInfo : pdfData) {
                    if (fileInfo.isChoosed()) {
                        allChoosedData.add(fileInfo);
                    }
                }
                for (FileInfo fileInfo : pptData) {
                    if (fileInfo.isChoosed()) {
                        allChoosedData.add(fileInfo);
                    }
                }
                for (FileInfo fileInfo : imageData) {
                    if (fileInfo.isChoosed()) {
                        allChoosedData.add(fileInfo);
                    }
                }
                ...
                intent.putParcelableArrayListExtra("datas", (ArrayList<? extends Parcelable>) allChoosedData);
